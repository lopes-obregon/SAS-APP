  ...
  function comparaDatas(data_selecionada_str = "") {
    ...
    if (json_datas != undefined) {
      json_datas?.datas.forEach((element) => {
        ...
        if (
          data_selecionada.getDate() == data_element.getDate() &&
          data_selecionada.getMonth() == data_element.getMonth()
        ) {
      ...
      Alert.alert(
        "Consulta",
        "Desculpe mas estamos consultado o banco de dados!"
      );
    }
    if (tem_valor > 0) {
      setHora(array_horas);
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  {parseInt(item) <= 12 ? (
                    <Text style={styles.title2}>AM</Text>
                  ) : (
                    <Text style={styles.title2}>PM</Text>
                  )}
                </TouchableOpacity>
              )}
            />
